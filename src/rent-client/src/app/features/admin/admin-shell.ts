...
/**
 * Port de Admin/_AdminLayout.cshtml: mismo esqueleto que los shells del renter y del
 * landlord (columna fija + drawer movil), con el sidebar y los textos del admin.
 */
@Component({
  selector: 'app-admin-shell',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [RouterOutlet, TranslocoPipe, Icon, AdminSidebar],
  template: `
      ...
      <div class="flex flex-col min-w-0">
        ...
        >
          ...
          >
            ...
            <span class="font-medium">{{ 'admin.title' | transloco }}</span>
          </button>
          <span class="text-xs text-slate-500 dark:text-white/50">{{ 'admin.role' | transloco }}</span>
        </div>
    ...
    >
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto">
          <app-admin-sidebar (navigated)="drawerOpen.set(false)" />
        </div>
      </aside>
    </div>
  `,
